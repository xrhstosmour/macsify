...
4. Headless browser script (Playwright or Puppeteer) that drives the UI, asserts on DOM/console/network.
...
7. Property or fuzz loop: if the bug is "sometimes wrong output", run 1000 random inputs and look for the failure mode.
...
Tool preference:
1. Debugger or REPL inspection if the env supports it. One breakpoint beats ten logs.
